...
class SAR(nn.Module):
    ...
    def forward(self, x):
        ...
        for _ in range(self.steps):
            outputs, features, ema, reset_flag = forward_and_adapt_sar(x, self.model, self.optimizer, self.margin_e0, self.reset_constant_em, self.ema)
            if reset_flag:
                self.reset()
...
def forward_and_adapt_sar(x, model, optimizer, margin, reset_constant, ema):
    ...
    return outputs, features, ema, reset_flag
